...
const DATASTORE_FOLDER = __dirname + '/../../datastore';
            ...
            return new Promise((resolve, reject) => {
                ...
                walk.sync(DATASTORE_FOLDER, (path, stats) => __awaiter(this, void 0, void 0, function* () {
                    if (path.endsWith('.db') &&
                        ...
                        this.collectionsInfo.push(Object.assign({ collection,
                            collectionTitle, rows: yield this.count(collection, {}) }, stats, { size: filesize(stats.size) }));
                    }
                    if (path.endsWith('.db') && path.indexOf('language/') >= 0) {
                        this.languageCount++;
                    }
...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        yield db.updateQuickStats();
        console.log(db.collectionsInfo);
        // for (let i = 0; i < 1024 * 1; i++)
        //   db.upsertLocale('en', faker.random.word(10), faker.random.words(200));
        // const entries = await db.find('language/en', {});
        // for (const entry of entries) {
        //   log.debug(`%s, ${entry.value}`, entry.key);
        //   console.log(await db.find(SCHEMA_COLLECTION, {}));
        //   break;
        // }
    });
}
